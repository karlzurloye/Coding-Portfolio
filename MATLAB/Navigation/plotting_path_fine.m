function [] = plotting_path_fine (path_A, path_B, path_C, occ, ref_A, ref_B, ref_C)
...
p1 = plot(path_A(:,2),path_A(:,3)); % driven paths
p1.LineWidth = 5;
...
hold on;
for ii = -2:0.1:4.9 % drawing occupied grid cells
    for jj = -6:0.1:5.9
        pos = [ii jj 0.1 0.1];
        if is_occupied_fine([ii jj],occ)
            rectangle('Position',pos,'FaceColor',[1 0 0],'LineWidth',0.5);
