...
int main (int argc, char *argv[])
{
    ...
    long heart_beat_timer=curr_time;
...
int sockfd = 0, n = 0;
    ...
    while(1)
    {
        alive = !alive; //toggles heartbeat
        shared_memory->b=alive; //updating shared memory
		...
		{
			thrust=1050-1.5*((float)recvBuff[0]-128.0);
	...
	// 
        if(curr_time>heart_beat_timer+1000)
        {
	 	shmdt(shared_memory);
		endwin();
		return 0;
